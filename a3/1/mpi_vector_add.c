...
void Check_for_error(int local_ok, char fname[], char message[],
      MPI_Comm comm);
// local_n_p is now an array of integers
// each indicates the size of vector i-th thread is responsible for
void Read_n(int* n_p, int** local_n_p, int** disps, int my_rank, int comm_sz,
      MPI_Comm comm);
// changed local_n -> *local_n, add n
void Allocate_vectors(double** local_x_pp, double** local_y_pp,
      double** local_z_pp, int local_n[], int my_rank, MPI_Comm comm);
void Read_vector(double local_a[], int local_n[], int disps[], int n,
      char vec_name[], int my_rank, MPI_Comm comm);
void Print_vector(double local_b[], int local_n[], int disps[],
      int n, char title[], int my_rank, MPI_Comm comm);
void Parallel_vector_sum(double local_x[], double local_y[],
      double local_z[], int local_n[], int my_rank);
...
int main(void) {
   ...
   Read_n(&n, &local_n, &disps, my_rank, comm_sz, comm);
#  ifdef DEBUG
   printf("Proc %d > n = %d, pos = ", my_rank, n);
   int i;
   for (i = 0; i < comm_sz; i++) printf("%d(@%d), ", local_n[i], disps[i]);
   printf("\n");
#  endif
   Allocate_vectors(&local_x, &local_y, &local_z, local_n, my_rank, comm);

   Read_vector(local_x, local_n, disps, n, "x", my_rank, comm);
   Print_vector(local_x, local_n, disps,  n, "x is", my_rank, comm);
   Read_vector(local_y, local_n, disps, n, "y", my_rank, comm);
   Print_vector(local_y, local_n, disps, n, "y is", my_rank, comm);

   Parallel_vector_sum(local_x, local_y, local_z, local_n, my_rank);
   Print_vector(local_z, local_n, disps, n, "The sum is", my_rank, comm);
...
void Read_n(
   ...
   // calculate local_n_p
   *local_n_p = malloc(comm_sz * sizeof(int));
   *disps = malloc(comm_sz * sizeof(int));
   if (*local_n_p == NULL || *disps == NULL) local_ok = 0;
   Check_for_error(local_ok, fname, "Can't allocate local vector(s)",
         comm);

   local_n = (*n_p)/comm_sz;
   left = (*n_p) - comm_sz * local_n;
   for (i = 0; i < left; i++) {
       (*local_n_p)[i] = local_n + 1;
   }
   for (; i < comm_sz; i++) {
       (*local_n_p)[i] = local_n;
   }
   (*disps)[0] = 0;
   for (i = 1; i < comm_sz; i++) {
       (*disps)[i] = (*disps)[i - 1] + (*local_n_p)[i - 1];
   }
}  /* Read_n */
 ...
 *            my_rank:  the rank of the thread
...
void Allocate_vectors(
   ...
   *local_x_pp = malloc(local_n[my_rank]*sizeof(double));
   *local_y_pp = malloc(local_n[my_rank]*sizeof(double));
   *local_z_pp = malloc(local_n[my_rank]*sizeof(double));
...
void Read_vector(
   ...
   if (my_rank == 0) {
      ...
      for (i = 0; i < n; i++)
         scanf("%lf", &a[i]);
      MPI_Scatterv(a, local_n, disps, MPI_DOUBLE, local_a, local_n[my_rank],
         MPI_DOUBLE, 0, comm);
      free(a);
   } else {
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
      ...
      //for (i = 0; i < local_n[my_rank]; i++) printf("%lf ", local_a[i]);
      //printf("\n");
   }
}  /* Read_vector */
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
      MPI_Gatherv(local_b, local_n[my_rank], MPI_DOUBLE, b, local_n,
            disps, MPI_DOUBLE, 0, comm);
      printf("%s\n", title);
      for (i = 0; i < n; i++)
         printf("%f ", b[i]);
      printf("\n");
...
void Parallel_vector_sum(
   ...
   for (local_i = 0; local_i < local_n[my_rank]; local_i++) {
      local_z[local_i] = local_x[local_i] + local_y[local_i];
      //printf("[%d] %d: %d = %d + %d\n", my_rank, local_i, local_z[local_i], local_x[local_i], local_y[local_i]);
    }
}  /* Parallel_vector_sum */
